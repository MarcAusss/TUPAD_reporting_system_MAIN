...
@section('content')
    <x-page-header eyebrow="Monitoring" title="PER PROVINCE (Current)"
            ...
            <table class="tupad-wide-table min-w-375 w-full text-xs">
                ...
                <tbody class="divide-y">
                    @foreach ($provinces as $p)
                        <tr>
                            <td class="p-3 font-bold text-[#063b86]">{{ $p['province'] }}</td>
                            @foreach (['allocation_total', 'target_grants'] as $k)
                                <td class="p-3 text-right">₱{{ number_format($p[$k], 2) }}</td>
                            @endforeach
                            <td class="p-3 text-right">
                                {{ number_format($p['target_beneficiaries']) }}</td>
                            <td class="p-3 text-right">₱{{ number_format($p['obligated_grants'], 2) }}</td>
                            <td class="p-3 text-right">{{ number_format($p['utilization'], 2) }}%</td>
                            @foreach (['wages', 'ppe', 'insurance'] as $k)
                                <td class="p-3 text-right">₱{{ number_format($p[$k], 2) }}</td>
                            @endforeach
                            <td class="p-3 text-right">
                                {{ number_format($p['beneficiaries']) }}</td>
                            <td class="p-3 text-right">{{ number_format($p['female']) }}</td>
                            @foreach (['unutilized', 'for_payment', 'post_docs', 'ongoing_implementation', 'for_implementation', 'approved', 'for_approval', 'under_evaluation', 'available_balance'] as $k)
                                <td class="p-3 text-right">₱{{ number_format($p[$k], 2) }}</td>
                            @endforeach
                        </tr>
                    @endforeach
