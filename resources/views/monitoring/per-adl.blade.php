                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse($rows as $r)
                        <tr class="hover:bg-blue-50/30">
                            <td class="p-3 font-bold text-[#063b86]">{{ $r['adl_number'] }}</td>
                            <td class="p-3">{{ $r['fund_sponsor'] }}</td>
                            <td class="p-3">{{ $r['lce_partylist'] }}</td>
                            <td class="p-3">{{ $r['province'] ?: '—' }}</td>
                            <td class="p-3">{{ $r['district'] ?: '—' }}</td>
                            <td class="p-3">{{ $r['municipality'] ?: '—' }}</td>
                            @foreach (['allocation_grants', 'allocation_admin_cost', 'allocation_total', 'realignment_grants'] as $k)
                                <td class="p-3 text-right">₱{{ number_format($r[$k], 2) }}</td>
                            @endforeach
                            <td class="p-3">
                                {{ $r['maf'] ?: '—' }}</td>
                            <td class="p-3 text-right">₱{{ number_format($r['target_grants'], 2) }}</td>
                            <td class="p-3 text-right">{{ number_format($r['target_beneficiaries']) }}</td>
                            <td class="p-3 text-right">₱{{ number_format($r['obligated_grants'], 2) }}</td>
                            <td class="p-3 text-right font-semibold">{{ number_format($r['utilization'], 2) }}%</td>
                            @foreach (['wages', 'ppe', 'insurance'] as $k)
                                <td class="p-3 text-right">₱{{ number_format($r[$k], 2) }}</td>
                            @endforeach
                            <td class="p-3 text-right">
                                {{ number_format($r['beneficiaries']) }}</td>
                            <td class="p-3 text-right">{{ number_format($r['female']) }}</td>
                            ...
                            <td colspan="33" class="p-0"><x-empty-state title="No monitoring rows found"
                                    message="Create or allocate an ADL to populate the current monitoring register." /></td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
