...
@section('content')
    ...
    <form method="GET" action="{{ route('project-workflow.index', ['queue' => $queue]) }}"
        ...
        <div class="flex flex-col gap-3 sm:flex-row">
            ...
            <div class="flex-1">
                ...
                <label for="workflow-search" class="sr-only">
                    Search workflow projects
                </label>

                <input id="workflow-search" name="q" value="{{ request('q') }}"
    ...
    @if ($queue === 'implementation')
        ...
        @php
            $forImplementation = $implementationBoard[\App\Enums\ProjectStatus::FOR_IMPLEMENTATION->value];

            $ongoingImplementation = $implementationBoard[\App\Enums\ProjectStatus::ONGOING_IMPLEMENTATION->value];

            $forPostDocuments = $implementationBoard[\App\Enums\ProjectStatus::FOR_SUBMISSION_OF_POST_DOCS->value];
            ...
            $boardColumns = [
                [
                    ...
                    'description' =>
                        'No work period yet, preparation is incomplete, or the approved Start Date has not arrived.',
                ...
                [
                    ...
                    'description' =>
                        'The implementation work period has ended and post-documentary requirements are now due.',
    ...
    ========================================================== --}}
            ...
            <div class="grid gap-5 xl:grid-cols-3">
                ...
                @foreach ($boardColumns as $column)
                    @php
                        $columnMeta = match ($column['key']) {
                            ...
                            'ongoing-implementation' => [
                                ...
                                'primary_button' =>
                                    'border border-[#063b86] bg-[#063b86] text-white hover:bg-[#052f6b]',
                    ...
                    <div class="relative min-w-0">
                        ...
                        @if (!$loop->last)
                            <div
                                class="pointer-events-none absolute -right-4.5 top-1/2 z-20 hidden -translate-y-1/2 xl:block">
                        ...
                        <div class="h-full rounded-xl border border-slate-200 bg-[#fbfcfe] p-3">
                            ...
                            <div class="mb-3 flex items-center justify-between gap-3 px-1 py-1">
                                ...
                                <div class="flex min-w-0 items-center gap-3">
                                    ...
                                    <div
                                        class="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg {{ $columnMeta['icon_bg'] }} {{ $columnMeta['icon_text'] }}">
                                ...
                                <span
                                    class="inline-flex h-6 min-w-6 items-center justify-center rounded-full px-2 text-[10px] font-bold {{ $columnMeta['count_bg'] }}">
                            ...
                            <div class="space-y-3">
                                ...
                                @forelse($column['projects'] as $project)
                                    ...
                                    <article
                                        ...
                                        <div class="mb-3">
                                            <span
                                                class="inline-flex rounded-md px-2 py-1 text-[9px] font-bold uppercase tracking-wide {{ $columnMeta['status_badge'] }}">
                                                {{ $statusLabel }}
                                            </span>
                                        </div>
                                        ...
                                        <dl class="space-y-2 text-[11px]">
                                            ...
                                            @if ($implementation)
                                                <div
                                                    class="grid grid-cols-[92px_minmax(0,1fr)] items-start gap-2 border-t border-slate-100 pt-2">
                                        ...
                                        <div class="mt-3 grid grid-cols-2 gap-2">
                                            ...
                                            @if (!$implementation && $project->status === \App\Enums\ProjectStatus::APPROVED)
                                                <a href="{{ route('projects.show', $project) }}#implementation"
                                                    class="inline-flex h-8 items-center justify-center rounded-md px-2 text-[10px] font-semibold {{ $columnMeta['primary_button'] }}">
                                                    Complete Requirements
                                                </a>
                                            @elseif(!$implementation && $project->status === \App\Enums\ProjectStatus::FOR_IMPLEMENTATION)
                                                <button type="button"
                                            ...
                                            @elseif($project->implementation_board_stage === \App\Enums\ProjectStatus::FOR_SUBMISSION_OF_POST_DOCS->value)
                                                <a href="{{ route('projects.show', $project) }}#post-documents"
                                                    class="inline-flex h-8 items-center justify-center rounded-md px-2 text-[10px] font-semibold {{ $columnMeta['primary_button'] }}">
        ...
        <div id="workPeriodModal" class="fixed inset-0 z-80 hidden" aria-hidden="true">
            <div class="absolute inset-0 bg-slate-950/50 backdrop-blur-[1px]" data-work-period-close></div>

            <div class="relative flex min-h-full items-center justify-center p-4">
                <div class="relative w-full max-w-xl overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl"
                    role="dialog" aria-modal="true" aria-labelledby="workPeriodModalTitle">
                    <div class="flex items-start justify-between gap-4 border-b border-slate-200 px-6 py-5">
                        <div>
                            ...
                            <h2 id="workPeriodModalTitle" class="mt-1 text-lg font-bold text-slate-900">
                                Set Work Period
                            </h2>
                        ...
                        <button type="button"
                            ...
                            aria-label="Close work period modal" data-work-period-close>
                            ×
                        </button>
                    </div>

                    <form id="workPeriodModalForm" method="POST" action="">
                        @csrf
                        ...
                        <div class="space-y-5 px-6 py-5">
                            ...
                            <div class="rounded-xl border border-slate-200 bg-slate-50 p-4">
                                <div class="grid gap-4 sm:grid-cols-2">
                                    <div>
                                        ...
                                        <div id="workPeriodProjectTitle"
                                            class="mt-1 text-sm font-semibold text-slate-900">
                                            —
                                        </div>

                                        <div id="workPeriodProjectCode" class="mt-1 text-xs text-slate-500">
                                            —
                                        </div>
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <div class="mt-1 flex items-center gap-2">
                                            <span id="workPeriodDuration" class="text-sm font-semibold text-slate-900">
                                                —
                                            </span>
                            ...
                            <div>
                                <label for="workPeriodStartDate" class="mb-2 block text-sm font-semibold text-slate-700">
                                    Start Date
                                    <span class="text-red-600">*</span>
                                </label>

                                <input id="workPeriodStartDate" name="start_date" type="date" required
                            ...
                            <div>
                                <label for="workPeriodEndDate" class="mb-2 block text-sm font-semibold text-slate-700">
                                    End Date
                                </label>
                            ...
                            <div>
                                <label for="workPeriodRemarks" class="mb-2 block text-sm font-semibold text-slate-700">
                                    Remarks
                                ...
                                <textarea id="workPeriodRemarks" name="remarks" rows="3" maxlength="3000"
                                    placeholder="Optional implementation-period remarks"
                                    class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-[#063b86] focus:outline-none focus:ring-2 focus:ring-blue-100"></textarea>
                            </div>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        ...
                        @forelse($projects as $project)
                            <tr class="hover:bg-slate-50/60">
                                ...
                                @if ($queue === 'for-compliance')
                                    @php
                                        $complianceEvaluation = $project->evaluations
