...
@section('content')
    ...
    */
        ...
        $nextAction = match ($project->status) {
            ...
            \App\Enums\ProjectStatus::TSSD_EVALUATION => 'Record the TSSD evaluation result.',

            \App\Enums\ProjectStatus::FOR_COMPLIANCE
                => 'Record the Date of Compliance. Saving moves the project directly to For Approval.',

            \App\Enums\ProjectStatus::FOR_APPROVAL => 'Complete the project approval action.',

            \App\Enums\ProjectStatus::APPROVED => 'Complete the implementation preparation requirements.',
            ...
            \App\Enums\ProjectStatus::FOR_PAYMENT => $project->obligation
            ...
            \App\Enums\ProjectStatus::COMPLETED => 'No pending workflow action. This project is complete.',
        };
    @endphp
    ...
    <div
        class="sticky top-16.25 z-20 mb-5 overflow-x-auto rounded-xl border border-slate-200 bg-white/95 p-2 shadow-sm backdrop-blur">

        <nav class="flex min-w-max items-center gap-1" aria-label="Project detail sections">

            <a href="#overview"
                class="rounded-lg px-3 py-2 text-xs font-semibold text-slate-600 hover:bg-slate-100 hover:text-slate-950">
                Overview
            </a>
    ...
    <section class="mt-5 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
        ...
        <div class="grid gap-4 p-5 sm:grid-cols-2">
            ...
            <div class="rounded-lg border border-slate-200 bg-slate-50 p-4">
                ...
                <div class="mt-2 text-2xl font-bold text-slate-900">{{ number_format($project->beneficiaries_female) }}
                </div>
            </div>
        </div>
    </section>
        ...
        <div class="p-5">
            ...
            @if ($project->status === \App\Enums\ProjectStatus::ONGOING_PROFILING)
                <div class="rounded-lg border border-amber-200 bg-amber-50 p-4">
                    ...
                    <p class="mt-1 text-xs leading-5 text-amber-800">
                        New project profiles move directly to TSSD Evaluation when saved. This Ongoing Profiling status is
                        retained only for older records created before the revised workflow.
                    </p>
                </div>
            @endif
            ...
            @if ($project->status === \App\Enums\ProjectStatus::FOR_APPROVAL)
                <div class="rounded-lg border border-emerald-200 bg-emerald-50 p-4">
                    ...
                    <p class="mt-1 text-xs leading-5 text-emerald-700">
                        Assign the official Project Code during approval. One project receives one Project Code, and that
                        code cannot be reused by another project.
                            ...
                            <tr>
                                ...
                                <td class="px-5 py-4">
                                    ...
                                    @if ($evaluation->result === 'for_compliance')
                                        @php
                                            $evaluationAging = $evaluation->compliance_date
                                                ? (int) $evaluation->evaluated_at
                                                    ->copy()
                                                    ->startOfDay()
                                                    ->diffInDays($evaluation->compliance_date->copy()->startOfDay())
                                                : null;
                                        @endphp
        ...
        <section id="implementation" class="scroll-mt-32 mt-5 rounded-xl border border-slate-200 bg-white shadow-sm">
            ...
            @if (in_array(
                ...
                <div class="grid gap-5 p-5 xl:grid-cols-2">
                    ...
                    <form method="POST" action="{{ route('projects.implementation.requirements', $project) }}"
                            ...
                            <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                                <div>
                                    ...
                                    <p class="mt-1 text-xs leading-5 text-slate-500">
                                        ...
                                        Saving all three complete requirements automatically moves the project to For
                                        Implementation.
                                    </p>
                                </div>
                        ...
                        <div class="grid gap-5 p-5 xl:grid-cols-3">
                            ...
                            <section class="rounded-xl border border-slate-200 p-5">
                                ...
                                <div class="mt-4 grid gap-4">
                                    ...
                                    <div>
                                        ...
                                        <input name="insurance[date_enrolled]" type="date" required
                                            value="{{ old('insurance.date_enrolled', $project->insuranceEnrollment?->date_enrolled?->format('Y-m-d')) }}"
                                            class="h-10 w-full rounded-lg border border-slate-300 px-3 text-sm">
                                    ...
                                    <div>
                                        ...
                                        <select name="insurance[payment_mode]" required
                                            class="h-10 w-full rounded-lg border border-slate-300 bg-white px-3 text-sm">
                                            <option value="">
                                                Select mode
                                            </option>

                                            <option value="voucher" @selected(old('insurance.payment_mode', $project->insuranceEnrollment?->payment_mode) === 'voucher')>
                                                Voucher
                                            </option>

                                            <option value="ca" @selected(old('insurance.payment_mode', $project->insuranceEnrollment?->payment_mode) === 'ca')>
                                                CA
                                            </option>
                                        </select>
                                    ...
                                    <div>
                                        ...
                                        <input name="insurance[or_number]"
                                            value="{{ old('insurance.or_number', $project->insuranceEnrollment?->or_number) }}"
                                            class="h-10 w-full rounded-lg border border-slate-300 px-3 text-sm">
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <input name="insurance[policy_number]"
                                            value="{{ old('insurance.policy_number', $project->insuranceEnrollment?->policy_number) }}"
                                            class="h-10 w-full rounded-lg border border-slate-300 px-3 text-sm">
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <textarea name="insurance[remarks]" rows="2"
                                            class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm">{{ old('insurance.remarks', $project->insuranceEnrollment?->remarks) }}</textarea>
                                    </div>
                            ...
                            <section class="rounded-xl border border-slate-200 p-5">
                                ...
                                <div class="mt-4">
                                    ...
                                    <input name="ppe[delivery_receipt_date]" type="date" required
                                        value="{{ old('ppe.delivery_receipt_date', $project->ppeDelivery?->delivery_receipt_date?->format('Y-m-d')) }}"
                                        class="h-10 w-full rounded-lg border border-slate-300 px-3 text-sm">
                                ...
                                <div class="mt-4">
                                    ...
                                    <textarea name="ppe[ppe_provided]" rows="5" required
                                        class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm">{{ old('ppe.ppe_provided', $project->ppeDelivery?->ppe_provided) }}</textarea>
                                ...
                                <div class="mt-4">
                                    ...
                                    <textarea name="ppe[remarks]" rows="2" class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm">{{ old('ppe.remarks', $project->ppeDelivery?->remarks) }}</textarea>
                                </div>
                            </section>
                            ...
                            <section class="rounded-xl border border-slate-200 p-5">
                                ...
                                <div class="mt-4 grid gap-4">
                                    <div>
                                        ...
                                        <input name="ntp[date_issued]" type="date" required
                                            value="{{ old('ntp.date_issued', $project->noticeToProceed?->date_issued?->format('Y-m-d')) }}"
                                            class="h-10 w-full rounded-lg border border-slate-300 px-3 text-sm">
                                    ...
                                    <div>
                                        ...
                                        <input name="ntp[date_released]" type="date" required
                                            value="{{ old('ntp.date_released', $project->noticeToProceed?->date_released?->format('Y-m-d')) }}"
                                            class="h-10 w-full rounded-lg border border-slate-300 px-3 text-sm">
                                ...
                                <div class="mt-4">
                                    ...
                                    <textarea name="ntp[remarks]" rows="2" class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm">{{ old('ntp.remarks', $project->noticeToProceed?->remarks) }}</textarea>
                                </div>
                            </section>
                        </div>
                        ...
                        <div
                            ...
                            <p class="text-xs leading-5 text-slate-500">
                                Saving updates all three requirements together. Once complete, status automatically becomes
                                For Implementation.
                            </p>
                        ...
                        <form method="POST" action="{{ route('projects.implementation.period', $project) }}"
                            ...
                            <div class="mt-4 grid gap-4 md:grid-cols-2">
                                ...
                                <div>
                                    ...
                                    <input id="implementation-end-date" type="date" readonly tabindex="-1"
    ...
    @elseif(in_array(
        ...
        <section id="implementation" class="scroll-mt-32 mt-5 rounded-xl border border-violet-200 bg-violet-50 p-5">
            ...
            <p class="mt-1 text-xs leading-5 text-violet-800">
                The Insurance, PPE, Notice to Proceed, Orientation, and Work Period forms in this implementation workflow
                ...
                <form method="POST" action="{{ route('projects.post-documents.store', $project) }}"
                    ...
                    <div class="grid gap-5 lg:grid-cols-3">
                        ...
                        <div class="lg:col-span-3">
                            ...
                            <textarea name="remarks" rows="3" class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm"
                                placeholder="Optional remarks about the received post-documentary requirements.">{{ old('remarks') }}</textarea>
                    ...
                    <div
                        ...
                        <p class="text-xs leading-5 text-slate-500">
                            A successful save records all selected attachments and automatically updates the project status
                            to
    ...
    @if (in_array($project->status, [\App\Enums\ProjectStatus::FOR_PAYMENT, \App\Enums\ProjectStatus::COMPLETED], true))
        ...
        <section id="payment" class="scroll-mt-32 mt-5 rounded-xl border border-slate-200 bg-white shadow-sm">
            ...
            <div class="border-b border-slate-200 px-5 py-4">
                ...
                <p class="mt-1 text-xs text-slate-500">
                    Focal/Admin records the wage payment or obligation information after post-documentary requirements are
                    completed.
                </p>
                    ...
                    <div class="grid gap-4 md:grid-cols-2">
                        ...
                        <div>
                            ...
                            <input name="month" required
                                value="{{ old('month', now()->format('F Y')) }}"
                                class="h-10 w-full rounded-lg border border-slate-300 px-3 text-sm">
