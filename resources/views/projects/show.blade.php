...
@php
    ...
    */
    ...
    $projectImplementationStarted = $project->implementation()->exists()
...
<div id="overview" data-workspace-panel="overview" class="scroll-mt-32 mt-5 grid gap-5 xl:grid-cols-2 {{ $workspace['default_tab'] !== 'overview' ? 'hidden' : '' }}">
    ...
    @if($project->status !== \App\Enums\ProjectStatus::COMPLETED)
        @php
            ...
            if ($isThroughAcp) {
                $completionLiquidationSummary = app(\App\Services\Projects\ProjectAcpLiquidationService::class)
                    ->summary($project);

                $completionChecklist = [
                    ['label' => 'ACP Payment', 'complete' => (bool) $project->acpPayment, 'tab' => 'workflow'],
                    ['label' => 'ACP Check Release', 'complete' => (bool) $project->acpCheckRelease, 'tab' => 'workflow'],
                    ['label' => 'ACP Implementation (Work Period)', 'complete' => (bool) $project->implementation, 'tab' => 'workflow'],
                    ['label' => 'ACP Liquidation (Fully Liquidated)', 'complete' => (bool) ($completionLiquidationSummary['is_fully_liquidated'] ?? false), 'tab' => 'financial'],
                ];
            } else {
                $completionPaymentSummary = app(\App\Services\Payments\ProjectPaymentService::class)
                    ->summary($project);

                $completionChecklist = [
                    ['label' => 'Insurance Enrollment', 'complete' => (bool) $project->insuranceEnrollment, 'tab' => 'workflow'],
                    ['label' => 'PPE Delivery', 'complete' => $project->ppeDeliveries->isNotEmpty(), 'tab' => 'workflow'],
                    ['label' => 'Notice to Proceed', 'complete' => (bool) $project->noticeToProceed, 'tab' => 'workflow'],
                    ['label' => 'Orientation', 'complete' => (bool) $project->orientation, 'tab' => 'workflow'],
                    ['label' => 'Implementation Period (Work Period)', 'complete' => (bool) $project->implementation, 'tab' => 'workflow'],
                    ['label' => 'Post-Documentary Requirements', 'complete' => $project->postDocumentsComplete(), 'tab' => 'workflow'],
                    ['label' => 'Payment of Wages (Fully Disbursed)', 'complete' => (bool) ($completionPaymentSummary['is_fully_paid'] ?? false), 'tab' => 'financial'],
                ];
            }
        ...
        <section class="overflow-hidden rounded-xl border {{ $completionRemaining > 0 ? 'border-amber-200' : 'border-emerald-200' }} bg-white shadow-sm xl:col-span-2">
            ...
            <div class="border-b border-slate-200 px-5 py-4">
                <div class="flex flex-wrap items-center justify-between gap-3">
                    ...
                    <span class="rounded-full px-3 py-1 text-xs font-bold {{ $completionRemaining > 0 ? 'bg-amber-100 text-amber-800' : 'bg-emerald-100 text-emerald-800' }}">
                ...
                @foreach($completionChecklist as $item)
                    <li class="flex items-center justify-between gap-3 px-5 py-3">
                        <div class="flex items-center gap-3">
                            <span class="flex h-6 w-6 shrink-0 items-center justify-center rounded-full text-xs font-bold {{ $item['complete'] ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-400' }}">
                                {{ $item['complete'] ? '✓' : '•' }}
                            </span>
                            <span class="text-sm {{ $item['complete'] ? 'text-slate-600' : 'font-semibold text-slate-900' }}">
                                {{ $item['label'] }}
                            </span>
                        </div>
                        ...
                        @unless($item['complete'])
                            <a
                                href="{{ route('projects.show', ['project' => $project, 'workspace' => $item['tab']]) }}"
    ...
    <section class="rounded-xl border border-slate-200 bg-white shadow-sm">
        ...
        @if($canManageProject && !$projectEditingLocked)
            <details class="border-t border-slate-200">
                ...
                <form
                    method="POST"
                    action="{{ route('projects.details.update', $project) }}"
                ...
                >
                    ...
                    @if($projectImplementationStarted)
                        <div class="mb-4 rounded-lg border border-blue-200 bg-blue-50 px-4 py-3 text-xs leading-5 text-blue-800">
                            Implementation has already started, so Duration, Wage Rate, Beneficiaries, and Insurance
                            Rate/Beneficiaries are locked here. Use Replace Beneficiary (Beneficiaries tab) to adjust
                    ...
                    <div class="grid gap-4 md:grid-cols-2">
                        ...
                        <div>
                            <label class="mb-2 block text-xs font-semibold text-slate-700">TEVS Date Verified</label>
                        ...
                        @unless($projectImplementationStarted)
                            ...
                            <div>
                                <label class="mb-2 block text-xs font-semibold text-slate-700">Duration (Number of Days)</label>
                            ...
                            <div>
                                <label class="mb-2 block text-xs font-semibold text-slate-700">Total Beneficiaries</label>
                            ...
                            <div>
                                <label class="mb-2 block text-xs font-semibold text-slate-700">Female Beneficiaries</label>
...
@if($canManageProject && !$projectEditingLocked)
    @php
        ...
        $replacementProvinceId = $project->province_id
            ?: $project->projectLocations()->orderBy('sort_order')->value('province_id');
    @endphp

    <section id="beneficiary-replacement" data-workspace-panel="beneficiaries" class="mt-5 overflow-hidden rounded-xl border border-blue-200 bg-white shadow-sm {{ $workspace['default_tab'] !== 'beneficiaries' ? 'hidden' : '' }}">
        ...
        <div class="border-b border-slate-200 px-5 py-4">
            ...
            <div class="mt-3 flex flex-wrap gap-4 text-xs text-slate-600">
                <span><span class="font-bold text-slate-900">{{ $project->beneficiaries->count() }}</span> encoded on roster</span>
                <span><span class="font-bold text-emerald-700">{{ $activeBeneficiaries->count() }}</span> active</span>
                <span><span class="font-bold text-amber-700">{{ $replacedCount }}</span> replaced historically</span>
            </div>
        </div>
        ...
        <details id="replaceBeneficiaryDetails">
            ...
            >
                ...
                @if($errors->has('reason') || $errors->has('removed_existing') || $errors->has('added') || $errors->has('insurance_beneficiaries') || $errors->has('beneficiary_addresses'))
                ...
                <div class="mt-5 grid gap-5 lg:grid-cols-2">
                    ...
                    <div class="rounded-xl border border-slate-200 bg-slate-50 p-4">
                        ...
                        @if($activeBeneficiaries->isNotEmpty())
                            <div class="mt-3 max-h-48 space-y-1.5 overflow-y-auto rounded-lg border border-slate-200 bg-white p-2">
                                @foreach($activeBeneficiaries as $beneficiary)
                                    <label class="flex items-center gap-2 rounded-md px-2 py-1.5 text-xs text-slate-700 hover:bg-slate-50">
                                        <input type="checkbox" name="removed_existing[]" value="{{ $beneficiary->id }}"
                ...
                <details class="mt-5 rounded-xl border border-slate-200">
                    ...
                    <div class="space-y-5 border-t border-slate-200 p-4">
                        ...
                        <div>
                            <label class="mb-2 block text-xs font-semibold text-slate-700">
                                Insurance Beneficiaries <span class="font-normal text-slate-400">(leave unchanged if not applicable)</span>
                            </label>
                            <div class="flex flex-wrap items-center gap-3">
                                <input type="number" id="replacementInsuranceBeneficiaries" name="insurance_beneficiaries"
                                    min="0" max="{{ $project->beneficiaries_total }}"
                        ...
                        <div>
                            ...
                            <div id="replacementAddressFields" class="mt-3 hidden rounded-lg border border-slate-200 bg-slate-50 p-3">
                                @if($replacementProvinceId)
                                    <div class="grid gap-3 sm:grid-cols-2">
                                        <div>
                                            <label class="mb-1.5 block text-[11px] font-semibold text-slate-600">Municipality / City</label>
                                            <select id="replacementMunicipality" class="h-9 w-full rounded-md border border-slate-300 bg-white px-2 text-xs">
                                                <option value="">Select municipality / city</option>
                                            </select>
                                        </div>
                                    </div>

                                    <div id="replacementBarangayOptions" class="mt-3 max-h-40 overflow-y-auto rounded-md border border-slate-200 bg-white p-2">
            ...
            */
            ...
            const setupRowList = (containerId, buttonId, fieldName, requirePwdFlags) => {
                ...
                button.addEventListener('click', () => {
                    ...
                    row.querySelectorAll('input, select').forEach(el => el.addEventListener('input', updateCountStatus));
            ...
            */
            ...
            function updateCountStatus() {
                ...
                if (removed === 0 && added === 0) {
                    status.className = 'mt-4 rounded-lg border border-amber-200 bg-amber-50 px-3 py-3 text-[11px] font-medium leading-5 text-amber-800';
                    status.textContent = 'Select/record who is being replaced, then add an equal number of replacement beneficiaries.';
                    return false;
                }

                if (removed !== added) {
                    status.className = 'mt-4 rounded-lg border border-amber-200 bg-amber-50 px-3 py-3 text-[11px] font-medium leading-5 text-amber-800';
                    status.textContent = `Being replaced: ${removed}. Replacement beneficiaries added: ${added}. These must match.`;
                    return false;
                }

                status.className = 'mt-4 rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-3 text-[11px] font-semibold leading-5 text-emerald-700';
                status.textContent = `Ready: ${removed} beneficiary(ies) will be replaced by ${added} new beneficiary(ies).`;
                return true;
            }

            form.querySelectorAll('.replacement-removed-checkbox').forEach(el => el.addEventListener('change', updateCountStatus));
            updateCountStatus();
            ...
            if (municipalitySelect && provinceId) {
                fetchJson(@json(route('locations.municipalities', $replacementProvinceId ?: 0)))
                    .then(municipalities => {
                        municipalities.forEach(item => {
                            ...
                            option.textContent = item.district ? `${item.name} — ${item.district}` : item.name;
                            municipalitySelect.appendChild(option);
                        });
                    })
                    .catch(() => {
                        barangayOptions.innerHTML = '<div class="px-2 py-4 text-center text-[11px] font-medium text-red-600">Unable to load municipalities.</div>';
                    });
                ...
                municipalitySelect.addEventListener('change', async () => {
                    ...
                    if (!municipalitySelect.value) {
                        barangayOptions.innerHTML = '<div class="px-2 py-4 text-center text-[11px] text-slate-400">Select a municipality/city first.</div>';
                        return;
                    }

                    barangayOptions.innerHTML = '<div class="px-2 py-4 text-center text-[11px] text-slate-400">Loading barangays...</div>';

                    try {
                        const barangays = await fetchJson(`/locations/municipalities/${municipalitySelect.value}/barangays`);
                        ...
                        barangays.forEach(barangay => {
                            ...
                            `;
                            label.querySelector('input').addEventListener('change', renderBarangayRows);
                            barangayOptions.appendChild(label);
                        });
                    } catch (error) {
                        barangayOptions.innerHTML = '<div class="px-2 py-4 text-center text-[11px] font-medium text-red-600">Unable to load barangays.</div>';
                    }
                });
                ...
                function renderBarangayRows() {
                    ...
                    const checked = Array.from(barangayOptions.querySelectorAll('.replacement-barangay-checkbox:checked'));
                    barangayRows.innerHTML = '';

                    checked.forEach((checkbox, index) => {
                        const barangayId = String(checkbox.value);
                        const current = existing.get(barangayId) ?? { total: '', female: '' };

                        const row = document.createElement('div');
                        row.className = 'replacement-barangay-row grid grid-cols-[minmax(0,1fr)_90px_90px] items-center gap-2 rounded-md border border-blue-100 bg-blue-50/50 p-2';
                    ...
                    <div class="p-4">
                        ...
                        >
                            ...
                            <div class="mt-4">
                                ...
                                >{{ old('compliance_remarks') }}</textarea>

                                <p class="mt-1 text-[11px] leading-4 text-amber-700">
                                    Describe the complied documents against the Required Documents listed above. This
                                    ...
                                    @if($project->ppeItems->isNotEmpty())
                                        ...
                                        <div class="grid gap-2 sm:grid-cols-2">
                                            @foreach($project->ppeItems as $ppeItem)
                                                ...
                                                <div class="ppe-item-toggle rounded-lg border border-slate-300 p-2.5 {{ $remaining <= 0 ? 'opacity-50' : '' }}">
                                                    <button
                                                        type="button"
                                                    ...
                                                    >
                                                        <span class="min-w-0 truncate">
                                                            {{ $ppeItem->product }}
                                                            <span class="font-normal text-slate-400">({{ $ppeItem->ppe_type->label() }})</span>
                                                        </span>
                                                        <span class="ppe-item-check hidden text-emerald-600">&check;</span>
                                                    </button>

                                                    <div class="mt-0.5 px-2 text-[10px] text-slate-400">
                                                        Remaining: {{ number_format($remaining) }} / {{ number_format($ppeItem->plannedQuantity()) }}
                                                    </div>
                            ...
                            <div class="mt-4 grid gap-4">
                                <div>
                                    ...
                                    <input
                                        ...
                                        value="{{ old(
                                            'date_issued',
                                            $project->noticeToProceed?->date_issued?->format('Y-m-d\TH:i')
                                        ) }}"
                                        class="h-10 w-full rounded-lg border border-slate-300 px-3 text-sm"
                                    >
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        value="{{ old(
                                            'date_released',
                                            $project->noticeToProceed?->date_released?->format('Y-m-d\TH:i')
                                        ) }}"
                                        class="h-10 w-full rounded-lg border border-slate-300 px-3 text-sm"
                                    >
                    ...
                    <div class="mt-4 grid gap-4 sm:grid-cols-2">
                        ...
                        <div>
                            ...
                            <input
                                ...
                                value="{{ old(
                                    ...
                                    $project->orientation?->beneficiaries_oriented
                                ) }}"
                                class="h-10 w-full rounded-lg border border-slate-300 px-3 text-sm"
                            >

                            <p class="mt-1 text-[11px] leading-4 text-slate-500">
                                Maximum: {{ number_format($project->beneficiaries_total) }} declared beneficiaries.
                            </p>
                        ...
                        <div>
                            ...
                            <input
                                ...
                                value="{{ old(
                                    'oriented_by',
                                    $project->orientation?->oriented_by
                                ) }}"
                                class="h-10 w-full rounded-lg border border-slate-300 px-3 text-sm"
                            >
...
@if($project->beneficiaryReplacements->isNotEmpty())
    <section data-workspace-panel="history" class="scroll-mt-32 mt-5 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm {{ $workspace['default_tab'] !== 'history' ? 'hidden' : '' }}">
        ...
        <div class="divide-y divide-slate-100">
            @foreach($project->beneficiaryReplacements as $replacement)
                <details class="group/replacement">
                    <summary class="flex cursor-pointer list-none flex-wrap items-center justify-between gap-3 px-5 py-4 marker:content-none [&::-webkit-details-marker]:hidden hover:bg-slate-50/60">
                        <div class="min-w-0">
