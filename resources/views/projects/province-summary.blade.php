...
@php
    ...
    $summaryPageTitle = $isSingleProjectSummary ? 'Project Summary' : $province->name . ' Province Summary';

    $summaryPageDescription = $isSingleProjectSummary
        ? $province->name .
            ' Project Summary — only the selected project is shown. Its municipalities and districts are expanded below.'
        : $province->name .
            ' Project Summary — official province project register with exact municipality / barangay beneficiary allocation.';
@endphp
...
@section('content')
    ...
    <x-page-header eyebrow="Project Summary" :title="$summaryPageTitle" :description="$summaryPageDescription">
        <x-slot:actions>
            @if ($isSingleProjectSummary)
                ...
                <a href="{{ route('project-summary.province', $province) }}"
    ...
    <section class="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
        ...
        <div class="flex flex-col gap-3 border-b border-slate-200 px-5 py-4 lg:flex-row lg:items-center lg:justify-between">
            ...
            <div class="flex flex-col gap-2 sm:flex-row">
                ...
                @unless ($isSingleProjectSummary)
                    <div class="relative">
                        <input id="projectRegisterSearch" type="search"
        ...
        <div class="overflow-x-auto">
            ...
            <table class="w-full min-w-375 table-fixed text-[10px] xl:min-w-0">
                ...
                <tbody id="projectRegisterBody" class="divide-y divide-slate-100">
                    ...
                    @forelse($projects as $project)
                        ...
                        <tr class="js-project-register-row hover:bg-slate-50"
                            data-search="{{ mb_strtolower(
                                collect([$project->approval?->project_code, $project->partner, $project->project_title])->filter()->implode(' '),
                            ) }}">
                ...
                @if ($projects->isNotEmpty())
                    <tfoot class="border-t-2 border-slate-300 bg-slate-50 font-bold text-slate-800">
                        ...
                        <tr>
                            <td colspan="3" class="border-r border-slate-200 px-2 py-3 text-right">
...
========================================================== --}}
    ...
    <section class="mt-5 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
            ...
            <div class="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
                ...
                <div>
                    ...
                    <p class="mt-1 text-xs leading-5 text-slate-500">
                        @if ($isSingleProjectSummary)
                            Only locations attached to this one project are shown, including all covered municipalities
                            across different districts.
                        @else
                            Only districts, municipalities/cities and barangays represented by current project locations are
                            shown.
                        @endif
                    </p>
                </div>
        ...
        <div class="border-b border-slate-200 px-5 py-3">
            ...
            <div class="flex flex-col gap-2 lg:flex-row lg:items-center lg:justify-between">
                ...
                <div class="relative w-full lg:w-105">
                    <input id="provinceHierarchySearch" type="search"
                ...
                <div class="flex gap-2">
                    ...
                    <button id="expandProvinceHierarchy" type="button"
                        class="h-10 rounded-lg border border-slate-300 bg-white px-3 text-xs font-semibold text-slate-700 hover:bg-slate-50">
                        Expand All
                    </button>

                    <button id="collapseProvinceHierarchy" type="button"
                        class="h-10 rounded-lg border border-slate-300 bg-white px-3 text-xs font-semibold text-slate-700 hover:bg-slate-50">
                        Collapse All
                    </button>
        ...
        <div id="provinceHierarchy" class="divide-y divide-slate-200">

            @forelse($districts as $districtIndex => $district)

                @php
                    $districtSearch = mb_strtolower(
                        collect([
                            $district['name'],
                            ...$district['municipalities']->pluck('name')->all(),
                            ...$district['municipalities']->pluck('barangays')->flatten(1)->pluck('name')->all(),
                ...
                <details class="js-province-district" data-search="{{ $districtSearch }}" open>
                    ...
                    <summary class="cursor-pointer list-none bg-[#fbfcfe] px-5 py-3">
                        ...
                        <div class="grid gap-3 md:grid-cols-[minmax(0,1fr)_130px_150px_170px] md:items-center">
                            ...
                            <div class="flex min-w-0 items-center gap-3">
                                ...
                                <span
                                    class="flex h-6 w-6 shrink-0 items-center justify-center rounded-md text-xs font-bold text-white {{ $districtTone['badge'] }}">
                                    {{ $districtIndex + 1 }}
                                </span>
                    ...
                    <div class="ml-5 border-l {{ $districtTone['line'] }}">
                        ...
                        @foreach ($district['municipalities'] as $municipality)
                            @php
                                $municipalitySearch = mb_strtolower(
                                    collect([
                                        $municipality['name'],
                                        ...$municipality['barangays']->pluck('name')->all(),
                                        ...$municipality['projects']->pluck('project_title')->all(),
                                    ])
                                        ->filter()
                                        ->implode(' '),
                                );
                            ...
                            <details class="js-province-municipality border-b border-slate-100 last:border-b-0"
                                data-search="{{ $municipalitySearch }}" open>
                                ...
                                <summary class="cursor-pointer list-none px-5 py-3 hover:bg-slate-50">
                                    ...
                                    <div class="grid gap-3 md:grid-cols-[minmax(0,1fr)_130px_150px_170px] md:items-center">
                                        ...
                                        <div class="flex min-w-0 items-center gap-3">
                                            ...
                                            <span class="-ml-5.25 w-5 border-t {{ $districtTone['line'] }}"></span>

                                            <svg class="h-4 w-4 shrink-0 text-slate-500" viewBox="0 0 24 24"
                                            ...
                                            <div>
                                                <div class="text-xs font-bold text-slate-800">
                                                    ...
                                                    @if ($municipality['is_city'])
                                                        <span
                                                            class="ml-1 rounded bg-violet-50 px-1.5 py-0.5 text-[8px] font-bold uppercase text-violet-700">
                                                            City
                                                        </span>
                                                    @endif
                                                </div>
                                ...
                                <div class="bg-slate-50/50 px-5 pb-4 pt-1">
                                        ...
                                        <div class="overflow-hidden rounded-lg border border-slate-200 bg-white">
                                            ...
                                            @foreach ($municipality['barangays'] as $barangay)
                                                <details
                                                    ...
                                                    data-search="{{ mb_strtolower(
                                                        collect([$barangay['name'], ...$barangay['projects']->pluck('project_title')->all()])->filter()->implode(' '),
                                                    ) }}">
                                                    ...
                                                    <summary
                                                        ...
                                                        <div
                                                            ...
                                                            <div class="flex min-w-0 items-center gap-2">
                                                                <span
                                                                    class="h-1.5 w-1.5 shrink-0 rounded-full bg-slate-400"></span>
                                                    ...
                                                    <div class="border-t border-slate-100 bg-slate-50 px-3 py-2">
                                                        ...
                                                        @foreach ($barangay['project_entries'] as $projectEntry)
                                                            ...
                                                            <div
                                                                class="flex flex-col gap-2 border-b border-slate-100 py-2 last:border-b-0 sm:flex-row sm:items-center sm:justify-between">
                                                                ...
                                                                <div>
                                                                    ...
                                                                    <div
                                                                        class="mt-0.5 text-[9px] {{ $projectEntry['is_exact'] ? 'font-semibold text-emerald-600' : 'text-amber-600' }}">
                                    ...
                                    @else
                                        <div
                                            class="rounded-lg border border-dashed border-slate-300 bg-white px-4 py-3 text-[10px] text-slate-500">
                                            This municipality is represented by project data, but no barangay-level location
                                            was recorded.
                                        </div>
                                    @endif
        ...
        @else
            <div class="border-t border-emerald-200 bg-emerald-50 px-5 py-3 text-[10px] leading-5 text-emerald-800">
                ...
                Barangay figures come from the saved project-location allocation, and municipality / district totals are
                rolled up from those barangay values.
            </div>
        @endif
