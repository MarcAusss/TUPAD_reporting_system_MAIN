...
@if (isset($report))
    @php
        ...
        $printDocumentControl = (array) ($report['document_control'] ?? []);
        $printReference = (string) ($printDocumentControl['reference'] ?? '—');
        $printVersion = (string) ($printDocumentControl['version_label'] ?? 'v1.0 · Rev 0');
        $printGeneratedBy = (string) ($printDocumentControl['generated_by'] ?? 'System-generated');
        $printClassification = (string) ($printDocumentControl['classification'] ?? 'Internal Government Report');
    @endphp
    ...
    <section class="official-print-header" aria-label="Official report identity">
        ...
        <div class="official-print-header__meta">
            <div class="official-print-header__meta-row">
                <span>Reference</span>
                <strong>{{ $printReference }}</strong>
            </div>
            <div class="official-print-header__meta-row">
                <span>Version</span>
                <strong>{{ $printVersion }}</strong>
            </div>
            <div class="official-print-header__meta-row">
                <span>Generated</span>
                <strong>{{ $printGeneratedAt->format('M d, Y h:i A') }}</strong>
            </div>
            <div class="official-print-header__meta-row">
                <span>Generated by</span>
                <strong>{{ $printGeneratedBy }}</strong>
            </div>
            <div class="official-print-header__meta-row">
                <span>Classification</span>
                <strong>{{ $printClassification }}</strong>
            </div>
        </div>
    </section>
@endif
