...
final class XlsxTableWriter
{
    ...
    private function worksheet(array $report): string
    {
        ...
        foreach ([
            'Document Reference' => (string) ($documentControl['reference'] ?? '—'),
            'Document Version' => (string) ($documentControl['version_label'] ?? 'v1.0 · Rev 0'),
            'Generated By' => (string) ($documentControl['generated_by'] ?? 'System-generated'),
            'Classification' => (string) ($documentControl['classification'] ?? 'Internal Government Report'),
        ] as $label => $value) {
            $rows[] = $this->row($rowNumber, [
                $this->inlineCell('A'.$rowNumber, $label, 2),
                $this->inlineCell('B'.$rowNumber, $value, 5),
            ]);
            $rowNumber++;
        }
        ...
        if (! empty($report['signatories'])) {
            $rowNumber++;
            $rows[] = $this->row($rowNumber++, [
                $this->inlineCell('A'.($rowNumber - 1), 'Official Signatories', 3),
            ]);

            foreach ($report['signatories'] as $signatory) {
                $rows[] = $this->row($rowNumber, [
                    $this->inlineCell('A'.$rowNumber, (string) ($signatory['label'] ?? ''), 2),
                    $this->inlineCell('B'.$rowNumber, (string) ($signatory['name'] ?? ''), 5),
                    $this->inlineCell('C'.$rowNumber, (string) ($signatory['position'] ?? ''), 5),
                    $this->inlineCell('D'.$rowNumber, (string) ($signatory['office'] ?? ''), 5),
                ]);
                $rowNumber++;
            }
        }

        $lastDataRow = max($headerRow, $headerRow + max(0, count($report['rows'])));
        $lastRow = max($lastDataRow, $rowNumber - 1);
        $lastColumn = $this->columnName(max($columnCount, 4));
        $columnXml = '';
        ...
        return '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>'
            ...
            .'<autoFilter ref="A'.$headerRow.':'.$this->columnName($columnCount).$lastDataRow.'"/>'
            .'</worksheet>';
    }
    ...
    private function coreProperties(array $report): string
    {
        ...
        return '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>'
            ...
            .'<dc:creator>'.$this->xml((string) data_get($report, 'document_control.generated_by', 'TUPAD Reporting System')).'</dc:creator>'
            .'<dc:subject>'.$this->xml((string) data_get($report, 'document_control.reference', 'Official TUPAD Report')).'</dc:subject>'
            .'<cp:keywords>'.$this->xml((string) data_get($report, 'document_control.version_label', 'v1.0 · Rev 0')).'</cp:keywords>'
