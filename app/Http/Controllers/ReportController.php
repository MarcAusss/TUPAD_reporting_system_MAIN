...
class ReportController extends Controller
{
    ...
    public function index(Request $request): View
    {
        ...
        return view('reports.index', [
            'report' => $this->documentControl->apply(
                $this->reports->generate($type, $dimension, $filters),
                $request->user(),
            ),
    ...
    public function exportCsv(Request $request): StreamedResponse
    {
        ...
        return response()->streamDownload(
            function () use ($report): void {
                ...
                fputcsv($handle, ['Document Reference', (string) ($documentControl['reference'] ?? '—')]);
                fputcsv($handle, ['Document Version', (string) ($documentControl['version_label'] ?? 'v1.0 · Rev 0')]);
                fputcsv($handle, ['Generated By', (string) ($documentControl['generated_by'] ?? 'System-generated')]);
                fputcsv($handle, ['Classification', (string) ($documentControl['classification'] ?? 'Internal Government Report')]);
                ...
                if (! empty($report['signatories'])) {
                    fputcsv($handle, []);
                    fputcsv($handle, ['Official Signatories']);
                    foreach ($report['signatories'] as $signatory) {
                        fputcsv($handle, [
                            (string) ($signatory['label'] ?? ''),
                            (string) ($signatory['name'] ?? ''),
                            (string) ($signatory['position'] ?? ''),
                            (string) ($signatory['office'] ?? ''),
                        ]);
                    }
                }
